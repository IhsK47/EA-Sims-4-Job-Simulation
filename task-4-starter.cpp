...
class Inventory {
private:
    std::vector<Item*> items; // change array to vector
    float total_money;
...
public:
    ...
    void add_item() {
        ...
        items.push_back(new Item(name, quantity, price)); // adding the item 
    }
    ...
    void remove_item(int item_index) {
        ...
        if (input_quantity <= quantity) {
            ...
            if (item->get_quantity() == 0) {
                ...
                items.erase(items.begin() + item_index); // Remove item from vector by resetting the iterator then going to its pos in vector
            }
